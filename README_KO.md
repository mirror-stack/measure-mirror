...
**[🦋 측정착시 도감 →](catalog/README_KO.md)** — 측정이 만든 이를 속인 실제 봉인 사례 81표본(게이밍·자가적발·거짓음성 가드·오염)
...
#   --new-ledger 는 원장이 아직 없을 때 **첫 봉인에만** 필요합니다.
#   그 뒤 명령은 붙이지 않아도 이어 씁니다(아래 "원장을 새로 만드는 건 명시적으로" 참조)
...
`--ledger` 의 기본값은 `./mm_ledger.jsonl` — **상대경로**입니다. 편리하지만, 원장이 사고로
태어나는 통로이기도 합니다. 이름을 잘못 치거나 같은 명령을 다른 디렉터리에서 돌리면
*두 번째* 원장이 조용히 생기고, 그건 디스크에서 볼 때 **일부러 만든 원장과 구별되지 않습니다.**

그래서 `mm` 은 **있는 원장에는 그대로 이어 쓰지만, 없는 원장을 말없이 만들지는 않습니다**:

```bash
mm register c1 ...                 # 원장 없음 → 거절 · 아무것도 안 쓴다
mm --new-ledger register c1 ...    # 명시하면 → 만든다
mm register c2 ...                 # 이제 있으니 → 플래그 없이 이어 쓴다
```

플래그는 하위명령 **앞뒤 어디에 붙여도** 먹고, 거절 메시지는 만들었을 경로를 **절대경로로**
찍습니다 — 틀린 게 "어느 디렉터리에 있었는가" 일 때 "그런 파일 없음" 은 도움이 안 되니까요.

> 이게 단정함의 문제가 아닌 이유 — **2026-08-26 실측**, 이 프로젝트 저자들이 쓰는 원장
> 디렉터리에는 원장 파일이 **92개** 쌓였는데 감사 설정이 이름으로 부르는 건 **4개** 였습니다. 서로 다른 두 프로젝트의
> 봉인이 `mm_ledger.jsonl` — 바로 이 기본값의 파일명 — 에 들어가 있었고, 세 번째 프로젝트가
> 헌장에 선언한 원장의 봉인 **62건**은 감사 대상 디렉터리 **밖**에 있었습니다. 그동안 무결성
> 검사는 내내 초록이었습니다. 사슬은 실제로 성했으니까요. **틀린 건 자리였고, 자리를 보는
> 검사는 없었습니다.**
