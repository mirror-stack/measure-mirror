...
## 판정 이력 — 🔴 KILL ×2 → 🟢 **홀드아웃 PASS** (2026-08-04)

| # | 봉인 | 세트 | 결과 | 결과 기록 |
|---|---|---|---|---|
| 1 | `98e993b2` | v04 개발셋 | 🔴 KILL · FP=2 | am `69c7ff9b` |
| 2 | `3e6bd450` | v04 개발셋 | 🔴 KILL · FP=1 | am `332ef52a` |
| 3 | `99a1a510` | **v01+v05 홀드아웃** | 🟢 **PASS · 22/22** | am `4927cc24` |

### 왜 3차가 증거력을 갖나
1·2차 수리는 전부 **v04에서 실패를 보고** 한 것이라, v04는 시험이 아니라 개발 신호로 소모됐다.
그래서 3차는 **개발이 한 번도 노출된 적 없는 v01·v05**에서 판정했다 — 같은 봉인런의 다른 두 홈에
**동일한 12개 편집 레시피를 그대로 재생**했으므로, 세트 간 차이가 설계 차이에서 올 수 없다.
봉인문에 **"이번이 마지막 수리 기회"** 를 사전 선언해 사후 재협상 여지를 없앴다.

⚠️ 개발셋 v04도 지금은 11/11이지만 **증거력이 없다**(고칠 때 본 데이터다). 판정은 홀드아웃 것이다.
